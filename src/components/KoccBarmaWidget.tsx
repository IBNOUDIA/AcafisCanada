...
export const KoccBarmaWidget: React.FC = () => {
  ...
  const handleSend = async (textToSend?: string) => {
    ...
    const userMsg: ChatMessage = {
      id: `user-${Date.now()}`,
      sender: "user",
      text: content || t("mentor.noMessageWithFile"),
    ...
    try {
      const res = await fetch("/api/mentor", {
        ...
        body: JSON.stringify({
          message: content || t("mentor.defaultFileQuestion"),
          topic: "general",
          ...(fileToSend
      ...
      const fallback = t("widget.fallbackReply");
      setMessages((prev) => [
        ...prev,
        {
          id: `mentor-${Date.now()}`,
  ...
  const getTranscript = () =>
    messages
      .map((m) => `${m.sender === "user" ? t("widget.transcriptMe") : "Kocc Barma"} (${m.timestamp}) : ${m.text}`)
      .join("\n\n");
      ...
      {isOpen && (
        <div className="fixed bottom-5 right-5 z-40 w-[92vw] max-w-sm h-[70vh] max-h-[600px] bg-slate-950 rounded-2xl border border-slate-800 shadow-2xl flex flex-col overflow-hidden">
            ...
            <div className="flex items-center gap-2.5 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <span className="text-[10px] text-emerald-400 flex items-center gap-1">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" /> {t("widget.online")}
                </span>
              </div>
            </div>
            <button
          ...
          <div className="px-3 py-2 bg-slate-900/60 border-b border-slate-800/80 flex items-center gap-1.5 shrink-0">
            ...
            <button
              onClick={handleCopy}
              title={t("widget.copyConversation")}
            ...
            <button
              onClick={handleShare}
              title={t("widget.shareConversation")}
              className="p-1.5 rounded-lg bg-slate-800 text-slate-400 hover:text-white cursor-pointer transition-colors"
            >
            ...
            <button
              onClick={handleClear}
              title={t("widget.deleteConversation")}
